...
echo "🚀 탄생 웹사이트 클라우드 배포 시작 (Version: latest_v58)..."
...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$CLOUD_USER@$CLOUD_SERVER" << 'EOF'
    ...
    if [ ! -d ".git" ]; then
        ...
        sudo git fetch --tags
        sudo git checkout tags/latest_v58
    else
        echo "🔄 최신 변경사항 가져오는 중..."
        ...
        sudo git checkout tags/latest_v55
        sudo git pull origin main
    fi

    echo "✅ Version latest_v55 체크아웃 완료"
