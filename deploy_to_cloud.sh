...
# 탄생(Tansaeng) 클라우드 자동 배포 스크립트 v9.0
# 사용법: ./deploy_to_cloud.sh

echo "🚀 탄생 웹사이트 클라우드 배포 시작 (Version: latest_v9)..."
...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$CLOUD_USER@$CLOUD_SERVER" << 'EOF'
    ...
    if [ ! -d ".git" ]; then
        ...
        sudo git fetch --tags
        sudo git checkout tags/latest_v9
    else
        echo "🔄 최신 변경사항 가져오는 중..."
    ...
    echo "✅ Version latest_v9 체크아웃 완료"
