...
## smartfarm-ui-2026-06-09 (2026-06-09) — 데몬 단일 제어 전환

### 🎛️ 자동제어 구조 개선
- **브라우저 자체 자동제어 비활성화** (`BROWSER_AUTO_CONTROL=false`)
  - 기존: 데몬과 브라우저가 둘 다 자동제어 명령 발행 → 접속 시 충돌
  - 접속 순간 브라우저가 모든 장치를 재평가/재전송 → 데몬이 켜둔 장치가
    OFF→ON 깜빡임 (특히 **히트펌프 압축기 급가동 위험** + 릴레이 마모)
  - HP/팬/천창/측창 4개 제어 루프에 가드 추가
  - 브라우저는 표시·수동조작만, 자동제어는 데몬 전담
- 유지: 화면 표시, 수동 버튼, 설정 저장 모두 정상 / 분무수경은 원래 데몬 전담

### 🏷️ 태그
- `smartfarm-ui-2026-06-09` (master 브랜치)
...
### 💧 포깅 습도 히스테리시스 추가
- 습도가 임계값(예 60%) 근처에서 출렁일 때 밸브가 급격히 ON/OFF 반복되던 문제
- 데드밴드 도입: 분무 멈춤 `습도 >= threshold`, 재개 `습도 < threshold - 5%`
- 그 사이 구간은 현재 상태 유지 → 채터링 방지 (`HUMIDITY_HYSTERESIS` 상수)
