...
- **수정**: 각 구역 컨트롤러(`ctlr-0004~0008`)의 `status` 토픽을 구독해 온라인 상태를 추적(`esp32Online`). `doSpray()`/`doStop()`에서 명령 발행 자체는 그대로 하되(재연결 시 다음 사이클부터 정상화), **오프라인으로 확인된 경우에만** DB 로그·텔레그램·성공 로그를 생략하고 데몬 자체 로그 파일에 "⚠️ 컨트롤러 오프라인 — 실제 작동 미확인"만 남기도록 변경. 상태 미수신(판단 불가) 시에는 기존과 동일하게 낙관적으로 기록.
- 밸브 안전 타임아웃/워치독 등 제어 로직 자체는 변경 없음 — 순수하게 "성공 보고" 조건만 실제 연결 상태에 맞게 고침.
- 커밋 `2026-08-01_0027`.
