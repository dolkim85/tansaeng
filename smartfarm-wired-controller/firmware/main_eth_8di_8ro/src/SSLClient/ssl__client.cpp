...
// ═══════════════════════════════════════════════════════════════════════════
// [tansaeng 프로젝트 로컬 수정, 2026-08-23] VENDORED_FROM.md 참고.
// 원본은 이 매크로를 스케치 쪽에서 정의해주길 기대했지만, Arduino 빌드에서
// 스케치의 #define은 라이브러리의 별도 translation unit(이 파일)에 전달되지
// 않는다(arduino-cli --verbose 컴파일 커맨드라인으로 실측 확인 — -DW5500_WORKAROUND
// 도 -D_W5500_H_도 존재하지 않았음). 그래서 이 로컬 사본에서는 무조건 활성화한다.
// ═══════════════════════════════════════════════════════════════════════════
#ifndef W5500_WORKAROUND
...
int start_ssl_client(
  ...
  const char **outFailedStep // [tansaeng 로컬 수정] 기본값은 헤더에 선언됨
) {
  // ESP-IDF log_* 매크로가 아니라 순수 Serial — Core Debug Level 설정과 무관하게
  // 항상 보인다. 이 줄이 실제로 뜨는지 자체가 "이 로컬 사본이 쓰이고 있다"는 증거.
  ...
  const char *stepName = "(unknown)"; // [tansaeng 로컬 수정] 실패 단계 추적

  do {
    ret = init_tcp_connection(ssl_client, host, port); // Step 1 - Initiate TCP connection
    if (ret != 0) {
      stepName = "init_tcp_connection(TCP 연결)";
      break;
    }
    ...
    if (ret == MBEDTLS_ERR_CTR_DRBG_ENTROPY_SOURCE_FAILED || ret != 0) {
      stepName = "seed_random_number_generator(난수생성기)";
      break;
    }
    ...
    if (ret != 0) { // MBEDTLS_ERR_XXX_ALLOC_FAILED undefined?
      stepName = "set_up_tls_defaults(TLS 기본설정)";
      break;
    }
    if (alpn_protos != NULL) {
      log_v("Setting ALPN protocols");
      if ((ret = mbedtls_ssl_conf_alpn_protocols(&ssl_client->ssl_conf, alpn_protos) ) != 0) {
        if (outFailedStep) *outFailedStep = "mbedtls_ssl_conf_alpn_protocols(ALPN)";
    ...
    if (ret != 0) {
      stepName = "auth_root_ca_buff(CA/PSK 인증설정)";
      break;
    }
    ...
    if (ret != 0) {
      stepName = "auth_client_cert_key(클라이언트 인증서/키)";
      break;
    }
    ...
    if (ret != 0) {
      stepName = "set_hostname_for_tls(SNI 설정)";
      break;
    }
    ...
    if (ret != 0) {
      stepName = "set_io_callbacks_and_timeout(IO콜백/타임아웃)";
      break;
    }
    ...
    if (ret != 0) {
      stepName = "perform_ssl_handshake(TLS handshake)";
      break;
    }
    int flags = verify_server_cert(ssl_client); // Step 8 - Verify the server certificate
    ret = flags;
    if (ret != 0) {
      stepName = "verify_server_cert(서버 인증서 검증)";
  ...
  if (outFailedStep) {
    *outFailedStep = (ret == 0) ? "(성공)" : stepName;
  }
  ...
  handle_error(ret);
  // [tansaeng 로컬 수정, 2026-08-23] 원본은 여기서 항상 0을 반환해 실패 원인을
  // 지웠다(SSLClient::connect()의 _lastError가 무조건 0이 되던 원인). 실제 실패
  // 코드(ret)를 그대로 돌려줘 상위(SSLClient::_lastError)까지 보존한다. 호출부의
  // "1이 아니면 실패"라는 판정 로직은 그대로 유효하다(0이든 ret든 1이 아니므로) —
  // 단, verify_server_cert()의 flags는 mbedtls 인증서 검증 비트마스크라 이론상
  // 정확히 1(MBEDTLS_X509_BADCERT_EXPIRED만 켜진 경우)이 될 수 있어, 그 경우
  // 성공 신호(1)와 혼동되지 않도록 -1로 치환한다.
  return (ret == 1) ? -1 : ret;
}
