...
int SSLClient::connect(const char *host, uint16_t port, const char *_CA_cert, const char *_cert, const char *_private_key)
{
  ...
  // [tansaeng 로컬 수정, 2026-08-23] &_lastFailedStep 추가 — 실패 단계 이름을 받아온다.
  int ret = start_ssl_client(sslclient, host, port, _timeout, _CA_cert, _use_ca_bundle, _cert, _private_key, NULL, NULL, _use_insecure, _alpn_protos, &_lastFailedStep);
  _lastError = ret;
