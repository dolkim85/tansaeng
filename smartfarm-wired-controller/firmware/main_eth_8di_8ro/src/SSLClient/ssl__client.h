...
// [tansaeng 로컬 수정, 2026-08-23 — VENDORED_FROM.md 참고] outFailedStep 파라미터 추가:
// 실패 시 어느 단계에서 실패했는지 문자열로 돌려받는다(원본은 항상 0으로 실패원인을
// 뭉갰음). 기본값이 있어 기존 호출부와 호환된다.
int start_ssl_client(sslclient__context *ssl_client, const char *host, uint32_t port, int timeout, const char *rootCABuff, bool useRootCABundle, const char *cli_cert, const char *cli_key, const char *pskIdent, const char *psKey, bool insecure, const char **alpn_protos, const char **outFailedStep = nullptr);
