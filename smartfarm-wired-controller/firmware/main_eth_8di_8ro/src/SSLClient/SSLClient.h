...
protected:
  ...
  int _lastError = 0;
  // [tansaeng 로컬 수정, 2026-08-23 — VENDORED_FROM.md] 실패 단계 이름. start_ssl_client()가
  // 채워준다(원본은 이 정보가 없었음).
  const char *_lastFailedStep = "(시도 없음)";
	int _peek = -1;
  uint32_t _timeout = 0;
...
public:
  ...
  // [tansaeng 로컬 수정, 2026-08-23] 마지막 connect() 실패가 어느 단계에서 났는지.
  const char* lastFailedStep() const { return _lastFailedStep; }
